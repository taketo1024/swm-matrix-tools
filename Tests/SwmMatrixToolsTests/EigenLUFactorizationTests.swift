...
class EigenLUFactorizationTests: XCTestCase {
    
    typealias R = RationalNumber
    typealias M<n: SizeType, m: SizeType> = MatrixIF<EigenMatrixImpl<R>, n, m>
    ...
    func testFactorizeRandom() {
        let n = 10
        let grid = (0 ..< n * n).map{ _ in R(Int.random(in: 0 ..< 5)) }
        let A: M<anySize, anySize> = .init(size: (n, n), grid: grid)
        let e = A.LUfactorize()
        let (P, Q, L, U) = e.PQLU
